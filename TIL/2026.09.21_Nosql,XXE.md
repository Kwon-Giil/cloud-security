...
XXE 실습 과정에서 HTTP/1.1 401 Unauthorized 응답이 반환됨. 

이후 다른 패킷의 요청 구조를 비교하던 중 Content-Type이 지정되지 않은 요청을 확인했고, 해당 요청에 XML 형식에 맞는 Content-Type과 XML Payload를 추가하여 실습을 진행함.
...
Juice Shop에서 업로드가 허용되는 파일 확장자를 선택해야 업로드 요청이 정상적으로 발생하는 것을 확인함
